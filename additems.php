<?php
   ...
   function random_str($length)
   {
    ...
    return $key;
   }//to generate a random string to put into Id column
   $success=0;
   $itemexists=0;
   $needforecho=0;
   $badexpiry=0;
   ...
   if(isset($_POST['submit']))
   {
      ...
      if($badexpiry==0)
      {
         ...
         {
            $checksql="SELECT product_name,Id,qty,addedon,expiry_date,price FROM product WHERE product_name='$item_name' AND s_id=$s_id";
            $response=@mysqli_query($dbc,$checksql);
            $Id=random_str(5);
            $query="SELECT Id from product";
            $Idcheck=@mysqli_query($dbc,$query);
            if(mysqli_num_rows($Idcheck)>0)//To get a unique Id
            {
            ...
            {
               $query="INSERT INTO product(Id,product_name,s_id,addedon,expiry_date,qty,price) VALUES('$Id','$item_name','$s_id',CURDATE(),'$expiry',$qty,$price)";
            ...
            else if(mysqli_num_rows($response))
            {
               ...
               if(strtotime($row['addedon'])==strtotime(date("Y-m-d")))
               {
                  ...
                  if(strtotime($old_expiry)==strtotime($expiry))
                  {
                     ...
                     {
                        $query="UPDATE product SET qty=$qty,price=$price WHERE Id='$Id'";
                     }
...
<html>
   ...
   <body>
      ...
      <h1 class="text-center">Fresh  Warehouse</h1>
