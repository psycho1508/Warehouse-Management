...
if(isset($_POST['submit']))
{
   ...
   else
   {
      ...
      {
         $query="SELECT delivery_date,qty,Id FROM orders WHERE order_id='$item'";
         ...
         if(mysqli_num_rows($dateQ)>0)
         {
            ...
            else
            {
               ...
               {
                  $query="UPDATE product SET qty=qty+'".$row['qty']."',sold=sold-'".$row['qty']."' WHERE Id='".$row['Id']."'";
                  $response=@mysqli_query($dbc,$query);
                  if(mysqli_affected_rows($dbc))
                  {
                     $success=1;
...
<html>
   ...
   <body>
      ...
      <h1 class="text-center">Product Warehouse</h1>
      ...
      <div class="centerFlex">
            <div class="container centerFlex">
               ...
               <div>
            <?php
            $query="SELECT order_id,orders.Id,orders.qty,orders.addedon,delivery_date,product_name,price,status FROM orders,product WHERE orders.s_id='$s_id' AND product.Id=orders.Id";
            $response=@mysqli_query($dbc,$query);
