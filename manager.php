...
<body>
   ...
   <h4 class="centerFlex">Orders:</h4>
      <div class="centerFlex ">
         <form class="frm-wel">
            <div>
               <?php
               $query="SELECT order_id,store_name,orders.Id,product.section,product.price,orders.qty,orders.addedon,delivery_date,status,address FROM product,users,orders WHERE users.s_id=orders.s_id AND product.Id = orders.Id ORDER BY delivery_date";
               $response=@mysqli_query($dbc,$query);
               if(mysqli_num_rows($response)==0)
               {
   ...
   <h4 class="centerFlex">Storage:</h4>
      <div class="centerFlex ">
         <form class="frm-wel">
            <div>
               <?php
               $query="SELECT Id,product_name,section,store_name,addedon,expiry_date,qty,price,sold FROM product,users WHERE users.s_id=product.s_id";
               $response=@mysqli_query($dbc,$query);
               if(mysqli_num_rows($response)==0)
               {
         ...
         <form class="frm-wel">
            <div>
               ...
               if(mysqli_num_rows($response)>0)
               {
                  $row=mysqli_fetch_array($response);
                  echo '<tr>
                  <td class="text-center">Total Number of Product Items in Storage</td>
                  <td class="text-center">'.$row['number'].'</td></tr>';
               }
               $query="SELECT MAX(qty)  as number,Id,product_name FROM product";
               $response=@mysqli_query($dbc,$query);
               if(mysqli_num_rows($response)>0)
               {
                  $row=mysqli_fetch_array($response);
                  ...
                  <td class="text-center">Qty:'.$row['number'].'<br> Id: '.$row['Id'].'<br>Name: '.$row['product_name'].'</td></tr>';
               }
               $query="SELECT MAX(sold) as number,Id,product_name FROM product";
               $response=@mysqli_query($dbc,$query);
               if(mysqli_num_rows($response)>0)
               {
                  $row=mysqli_fetch_array($response);
                  ...
                  <td class="text-center">Sold:'.$row['number'].'<br> Id: '.$row['Id'].'<br>Name: '.$row['product_name'].'</td></tr>';
               }
               $query="SELECT COUNT(product.s_id),store_name FROM product,users WHERE product.s_id=users.s_id GROUP BY store_name";
               $response=@mysqli_query($dbc,$query);
               if(mysqli_num_rows($response)>0)
               {
                  $row=mysqli_fetch_array($response);
   ...
   <script>
      ...
      <?php 
            $query = "SELECT SUM(qty) as result FROM product GROUP BY section ORDER BY section ASC";
            $response = @mysqli_query($dbc, $query);
      ?>
