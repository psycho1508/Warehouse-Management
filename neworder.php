   ...
   else
   {
      foreach ($_POST['check_list'] as $item)
      {
         ...
         foreach ($_POST['qty_list'][$item] as $qty)
         {
            ...
            $query="INSERT INTO orders(order_id,s_id,Id,qty,addedon,delivery_date) VALUES('$order_id',$s_id,'$item',$qty,CURDATE(),'$delivery_date')";
            mysqli_query($dbc,$query);
            if(mysqli_affected_rows($dbc))
            {
               $query="UPDATE food SET qty=qty-$qty,sold=sold+$qty WHERE Id='$item'";
               mysqli_query($dbc,$query);
               if(mysqli_affected_rows($dbc))
               {
                  $success=1;
...
<html>
   ...
   <body>
      ...
      <h1 class="text-center"> Welcome To Warehouse</h1>
      ...
      <div class="centerFlex">
            <div class="container centerFlex">
               ...
               <div>
                  <?php
               $query="SELECT Id,product_name,qty,price,expiry_date FROM product";
               $response=@mysqli_query($dbc,$query);
               ...
               if($num_rows>0){
                  ...
                  while($row=mysqli_fetch_array($response))
                  {
                     ...
                     if($row['qty']>0)
                     {
                        echo '<input type="number" min="0" max="'.$row['qty'].'" name="qty_list['.$row['Id'].'][]" placeholder="Qty">/'.$row['qty'];
                     }
                     else
                     {
                        echo '<input type="number" min="0" max="'.$row['qty'].'" name="qty_list['.$row['Id'].'][]" placeholder="Qty" disabled>/'.$row['qty'];
                     }
                     echo '</td>
                     <td class="text-center">'.$row['price'].'</td>
                     <td class="text-center">'.$row['expiry_date'].'</td>
                     <td class="text-center">
                     <label class="checkbox-inline"><input type="checkbox" name="check_list['.$row['Id'].']" value="'.$row['Id'].'"></label>
                     </td>
                     </tr>';
                  }
                  echo '</table>';
               ...
               <div class="text-center col-md-12">
                  <button type="submit" name="submit" class="btn btn-danger">Add</button><br><br>
          <button class="btn btn-info m-9" type="submit" formaction="pay.php" value="">Procced to Pay</button> 
               </div>
            </form>
            </div>
         </form>
